...
class MiniSpider(PicoSpider):
    ...
    rules = (
        Rule(SLE(allow=torrent_links, deny=deny_rules), callback='parse_mininova_torrent', follow=True),
        Rule(SLE(allow=category_links, deny=deny_rules), callback='parse_category', follow=True)
    )
